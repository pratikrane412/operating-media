    ...
    >
      ...
      <div className="relative aspect-[9/14] rounded-2xl overflow-hidden bg-gray-100 shadow-md">
        ...
        <div className="absolute top-3 left-3">
          <span className="bg-[#FF5A1F] text-white text-[10px] font-bold tracking-[0.1em] px-2.5 py-1 rounded-lg">
            {video.tag}
          </span>
        </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            <Play size={20} className="text-[#FF5A1F] ml-1" fill="#FF5A1F" />
          </motion.div>
        </div>

        {/* Bottom quote overlay */}
        <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent px-4 pt-10 pb-4 transition-all duration-400 ${hovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}>
          <div className="flex gap-1.5 mb-1.5">
            <Quote size={12} className="text-[#FF5A1F] shrink-0 mt-0.5" fill="#FF5A1F" />
            <p className="text-white/90 text-[11.5px] leading-relaxed italic">{video.quote}</p>
          </div>
        </div>

        {/* OM logo watermark bottom-right */}
        <div className="absolute bottom-3 right-3 bg-white/20 backdrop-blur-sm rounded-lg px-2 py-1">
          <span className="text-white text-[9px] font-bold tracking-widest uppercase">OM</span>
        </div>
      </div>
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          <h4 className={`text-[14px] font-bold tracking-tight leading-tight transition-colors duration-200 ${hovered ? 'text-[#FF5A1F]' : 'text-gray-950'}`}>
            {video.name}
          </h4>
          <p className="text-[12px] text-gray-400 font-normal mt-0.5">{video.role}</p>
        </div>
        <ArrowUpRight
          size={15}
          strokeWidth={2}
...
export default function LearningSection() {
  ...
  return (
    <>
      <section className="relative bg-white py-10 px-6 overflow-hidden font-['Inter',sans-serif]">
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          ...
          >
            <div className="flex justify-center items-center gap-3 mb-4">
              ...
              <span className="text-[11px] font-bold tracking-[0.18em] uppercase text-[#FF5A1F]">
            ...
            <p className="text-gray-400 text-[15px] font-normal max-w-sm mx-auto leading-relaxed">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="mt-14 flex flex-col sm:flex-row items-center justify-between gap-2 pt-0 border-t border-gray-100"
          >
            <p className="text-gray-600 text-sm">500+ students placed across top agencies & brands</p>
            <a
              href="https://www.youtube.com/@operatingmedia"
              target="_blank"
              rel="noopener noreferrer"
              className="group flex items-center gap-2 bg-gray-950 text-white text-[13px] font-bold px-7 py-3.5 rounded-xl hover:bg-[#FF5A1F] transition-all duration-200"
            >
              Watch More Stories
