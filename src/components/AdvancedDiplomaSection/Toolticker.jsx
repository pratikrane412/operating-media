...
const dmTools = [
    ...
    { name: "Google Analytics", src: "https://www.operatingmedia.com/wp-content/uploads/2024/06/Group-145.png" },
];

const LogoCard = ({ tool, index }) => (
    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: index * 0.01 }} viewport={{ once: true }} className="bg-white border-r border-b border-gray-100 flex items-center justify-center p-4 h-[65px] md:h-[90px] group hover:bg-gray-50 transition-colors">
        <img src={tool.src} alt={tool.name} className="max-w-full max-h-[25px] md:max-h-[35px] object-contain transition-all duration-300 group-hover:scale-110" loading="lazy" />
    </motion.div>
);

export function ToolsTicker() {
    return (
        <section className="relative w-full py-13 bg-[#FAFAFA]" style={satoshi}>
            <div className="absolute inset-0 opacity-[0.25] pointer-events-none" style={{ backgroundImage: `radial-gradient(#000 1px, transparent 1px)`, backgroundSize: '30px 30px' }} />
            <div className="relative z-10 max-w-7xl mx-auto px-6">
                <div className="text-center mb-16">
                    <h2 className="font-black text-gray-950 mb-4 tracking-tight" style={{ ...satoshi, fontSize: 'clamp(24px, 4vw, 40px)' }}>
                        Master <span className="text-[#F26522]">60+ Industry</span> Leading Tools
                    </h2>
                    ...
                    <p className="text-gray-500 font-medium max-w-2xl mx-auto leading-relaxed" style={{ ...satoshi, fontSize: 'clamp(16px, 2vw, 18px)' }}>
                        Get hands-on experience with the exact software stack used by top agencies and AI-driven companies.
                    </p>
                </div>

                <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
                    <div className="bg-gray-50 px-8 py-4 border-b border-gray-100 flex items-center justify-between">
                        <span className="font-black uppercase text-[#F26522]" style={{ ...satoshi, fontSize: 12, letterSpacing: '0.2em' }}>Section 01: AI Powered Tools</span>
                    </div>
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 border-l border-t border-gray-100">
                        {aiTools.map((tool, idx) => (<LogoCard key={`ai-${idx}`} tool={tool} index={idx} />))}
                    </div>
                    <div className="bg-gray-50 px-8 py-4 border-b border-t border-gray-100 flex items-center justify-between mt-0">
                        <span className="font-black uppercase text-[#F26522]" style={{ ...satoshi, fontSize: 12, letterSpacing: '0.2em' }}>Section 02: Digital Marketing Stack</span>
                    </div>
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-9 border-l border-t border-gray-100">
                        {dmTools.map((tool, idx) => (<LogoCard key={`dm-${idx}`} tool={tool} index={idx} />))}
                    </div>
                </div>

                <div className="mt-10 flex flex-col items-center gap-4">
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="bg-[#F26522] text-white font-black px-7 py-3.5 rounded-lg shadow-[0_10px_30px_rgba(242,101,34,0.3)] flex items-center gap-3 hover:bg-gray-950 transition-all duration-300 group border-none cursor-pointer" style={{ ...satoshi, fontSize: 15, letterSpacing: '0.05em' }}>
                        <Download size={20} className="group-hover:translate-y-0.5 transition-transform" />
                        DOWNLOAD COMPLETE TOOLS SYLLABUS
                    </motion.button>
                    <p className="text-gray-400 font-bold uppercase" style={{ ...satoshi, fontSize: 11, letterSpacing: '0.18em' }}>Updated for 2026 Industry Standards</p>
                </div>
            </div>
        </section>
    );
