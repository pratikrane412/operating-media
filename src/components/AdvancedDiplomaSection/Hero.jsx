...
const features = [
    ...
    "Content Marketing",
    "Canva Design Tools",
    "Real Ad Spend Practice",
];

const stats = [
    { value: "120+", label: "Hours of Learning" },
    { value: "60+", label: "Tools Covered" },
    { value: "0%", label: "EMI Interest" },
    { value: "15", label: "Batch Size" },
        ...
        <section className="relative w-full overflow-hidden bg-[#FBF8F4]" style={S}>
                    ...
                    <div>
                        ...
                        <div style={anim(80)}>
                            ...
                            >
                                Advanced<br />
                                Diploma in<br />
                                <span style={{ color: "#FF5A1F" }}>Digital Marketing</span><br />
                        ...
                        <div style={anim(160)}>
                            <p style={{ ...S, fontWeight: 400, fontSize: "18px", color: "rgba(17,17,17,0.72)", lineHeight: 1.75, marginBottom: "40px", maxWidth: "500px" }}>
                                ...
                                the only diploma course in Mumbai where we fund your campaigns.
                            </p>
                        </div>
                    ...
                    <div style={anim(160)} className="relative">
                        <div className="bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-[0_24px_64px_rgba(0,0,0,0.10)]">
                            <img
                                ...
                                alt="Advanced Diploma Program"
                                className="w-full object-cover"
                            />
                            <div className="px-6 py-5 grid grid-cols-3 divide-x divide-gray-100 bg-white">
                                {[{ v: '4', l: 'Months' }, { v: '120+', l: 'Hours' }, { v: '60+', l: 'Tools' }].map((s, i) => (
                                    <div key={i} className={`flex flex-col items-center ${i > 0 ? 'pl-4' : ''}`}>
                                        <span style={{ ...S, fontWeight: 800, fontSize: "22px", color: "#111111", lineHeight: 1 }}>{s.v}</span>
                                        <span style={{ ...S, fontWeight: 500, fontSize: "11px", color: "rgba(17,17,17,0.4)", textTransform: "uppercase", letterSpacing: "0.08em", marginTop: "4px" }}>{s.l}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className="absolute -bottom-5 -right-4 z-20 bg-[#FF5A1F] text-white rounded-2xl px-5 py-3 shadow-xl shadow-orange-200">
                            <p style={{ ...S, fontWeight: 700, fontSize: "14px", lineHeight: 1, color: "#fff" }}>100% Placement</p>
                            <p style={{ ...S, fontWeight: 400, fontSize: "12px", color: "rgba(255,255,255,0.75)", marginTop: "3px" }}>Industry Recognised</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="relative z-10 border-t border-gray-200 bg-white/80 backdrop-blur-sm">
                <div className="max-w-7xl mx-auto px-10 lg:px-20 py-9 flex flex-wrap justify-between gap-6">
                    {[
                        ...
                        { icon: <Zap size={17} />, title: "AI-Powered", sub: "ChatGPT & Canva AI" },
                        { icon: <ShieldCheck size={17} />, title: "100% Placement", sub: "Job Support" },
                        { icon: <Users size={17} />, title: "Small Batches", sub: "Only 15 Students" },
                    ].map((s, i) => (
                        <div key={i} className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-[#FF5A1F]/10 flex items-center justify-center text-[#FF5A1F]">{s.icon}</div>
                            <div>
                                <p style={{ ...S, fontWeight: 700, fontSize: "14px", color: "#111111", lineHeight: 1.2 }}>{s.title}</p>
                                <p style={{ ...S, fontWeight: 400, fontSize: "12px", color: "rgba(17,17,17,0.5)", marginTop: "2px" }}>{s.sub}</p>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-14 py-20">
                <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10">
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span style={{ ...S, fontWeight: 700, fontSize: "11px", color: "#FF5A1F", letterSpacing: "0.35em", textTransform: "uppercase" }}>Curriculum</span>
                        </div>
                        <h2 style={{ ...S, fontWeight: 900, fontSize: "clamp(28px, 3.5vw, 46px)", color: "#111111", lineHeight: 1.1, letterSpacing: "-0.03em" }}>
                            What you will{' '}<span style={{ color: "#FF5A1F" }}>achieve</span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {features.map((feat, i) => (
                        <div key={i} className="group flex items-start gap-3 p-4 rounded-2xl border border-gray-100 bg-white hover:border-[#FF5A1F]/30 hover:shadow-md transition-all duration-200 cursor-default shadow-sm">
                            <CheckCircle2 size={16} className="text-[#FF5A1F] mt-0.5 shrink-0" />
                            <span style={{ ...S, fontWeight: 500, fontSize: "14px", color: "rgba(17,17,17,0.7)", lineHeight: 1.45 }} className="group-hover:text-[#111111] transition-colors">{feat}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-14 grid grid-cols-2 md:grid-cols-5 gap-6 pt-10 border-t border-gray-200">
                    {stats.map((s, i) => (
                        <div key={i}>
                            <span style={{ ...S, fontWeight: 900, fontSize: "clamp(1.6rem, 3vw, 2.2rem)", color: "#111111", lineHeight: 1, display: "block", letterSpacing: "-0.03em" }}>{s.value}</span>
                            <span style={{ ...S, fontWeight: 600, fontSize: "11px", color: "rgba(17,17,17,0.45)", textTransform: "uppercase", letterSpacing: "0.12em", marginTop: "8px", display: "block" }}>{s.label}</span>
                        </div>
                    ))}
                </div>
            </div>
