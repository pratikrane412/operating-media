...
function FeaturePill({ iconName, text }) {
  return (
    <div className="flex items-center gap-2.5 px-4 py-2.5 bg-blue-50/50 rounded-full
                    border border-blue-100 shadow-[0_2px_8px_rgba(37,99,235,0.05)] hover:border-[#2563eb]/30 transition-colors">
      <span className="text-[#2563eb]" aria-hidden="true">
        <Icon name={iconName} size={15} />
      </span>
      <span className="text-[14px] font-bold text-[#0f172a] whitespace-nowrap">
        {text}
      </span>
    </div>
  );
  ...
  return (
    <motion.div
      ...
      className="group relative p-6 rounded-2xl bg-white border border-gray-200
                 ...
                 hover:shadow-[0_10px_30px_-10px_rgba(37,99,235,0.15)]
                 hover:-translate-y-1
                 transition-all duration-300 cursor-default overflow-hidden"
    >
      <div
        className="absolute inset-0 rounded-2xl pointer-events-none bg-gradient-to-br from-[#2563eb]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"
        aria-hidden="true"
      />
      <div
        className="w-12 h-12 rounded-xl flex items-center justify-center
                   text-[#2563eb] bg-blue-50 mb-5
                   group-hover:bg-[#2563eb] group-hover:text-white group-hover:scale-110
                   transition-all duration-300"
        aria-hidden="true"
      >
        <Icon name={iconName} size={22} />
      </div>
      <p className="text-[18px] font-black text-[#0f172a] mb-2 relative z-10">
        {title}
      </p>
      <p className="text-[15px] font-medium text-gray-500 leading-relaxed relative z-10">
        {desc}
      </p>
    </motion.div>
  );
...
function SpecRow({ iconName, label, value, delay }) {
  return (
    ...
    >
      ...
      <div>
        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-[0.15em] mb-0.5">
          {label}
        </p>
          ...
          >
            ...
            <div className="hidden md:block md:w-1/2 relative min-h-[480px]">
              ...
              <div className="absolute bottom-10 left-10 right-8">
                <p className="text-[24px] font-black text-white leading-tight mb-1">
                  Visit our campus.
                </p>
                <p className="font-medium text-[16px] text-[#ECAB00]">
                  Andheri &amp; Borivali
                </p>
              </div>
            </div>
            ...
            <div className="relative w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-[#FAFCFF]">
              ...
              <button
                onClick={onClose}
                className="absolute top-5 right-5 w-10 h-10 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:bg-gray-100 hover:text-[#0f172a] transition-colors cursor-pointer shadow-sm"
              >
                <X size={20} aria-hidden="true" />
              </button>

              <p className="text-[11px] font-bold text-[#2563eb] tracking-[0.25em] uppercase mb-4">
                Book a Visit
              </p>

              <h2 className="text-[32px] sm:text-[40px] font-black text-[#0f172a] leading-[1.1] tracking-tight mb-8">
                Book a Free<br />
                <span className="text-[#ECAB00]">Demo Class</span>
              </h2>
              ...
              <div className="space-y-5">
                <div className="flex items-center overflow-hidden rounded-xl bg-white border border-gray-200 focus-within:border-[#2563eb] focus-within:ring-2 focus-within:ring-[#2563eb]/20 transition-all shadow-sm">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3.5 outline-none text-[16px] font-medium text-[#0f172a] placeholder-gray-400 bg-transparent"
                  />
                </div>
                ...
                <button
                  onClick={() => { }}
                  className="group w-full flex items-center justify-center gap-2 py-4 rounded-xl cursor-pointer bg-[#ECAB00] hover:bg-[#0f172a] text-[#0f172a] hover:text-white text-[16px] font-black shadow-[0_10px_20px_-10px_rgba(236,171,0,0.5)] hover:shadow-xl transition-all duration-300"
                >
                  Continue
        ...
        <div className="grid lg:grid-cols-[1fr_450px] gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:sticky lg:top-24 self-start relative z-20">
            <motion.div
              ...
              className="relative bg-white rounded-[2rem] overflow-hidden border border-gray-200 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)]"
            >

              {/* ── Dark Navy Header ── */}
              <div className="bg-[#0f172a] px-8 pt-8 pb-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-[300px] h-[300px] bg-[#2563eb]/20 rounded-full blur-[60px] pointer-events-none" aria-hidden="true" />
                ...
                <p className="text-[11px] font-bold text-[#ECAB00] uppercase tracking-[0.3em] mb-2 relative z-10">
                  Enrolment
                </p>
                <h3 className="text-[28px] md:text-[32px] font-black text-white leading-[1.1] relative z-10">
                  Batch Details
                </h3>
                <p className="text-[15px] font-medium text-gray-400 mt-2 relative z-10">
                ...
                <div className="grid grid-cols-3 gap-3 mt-8 relative z-10">
                  {BATCH_STATS.map((s) => (
                    <div key={s.label} className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center backdrop-blur-sm">
                      <span className="text-[28px] font-black text-white leading-none block">
                        <Counter target={s.target} suffix={s.suffix} />
                      </span>
                      <span className="text-[10px] font-bold text-[#ECAB00] uppercase tracking-[0.1em] mt-2 block">
                        {s.label}
                      </span>
                    </div>
                  ))}
              ...
              <div className="px-6 pt-5 pb-3 space-y-1">
                {SPECS.map((item, i) => (
                  <SpecRow key={item.label} {...item} delay={0.2 + i * 0.07} />
                ))}
              </div>
                ...
                <div className="bg-[#FAFCFF] rounded-2xl p-5 space-y-3 border border-blue-50">
                  {CHECKLIST.map((text) => (
                    <div key={text} className="flex items-start gap-3">
                      <CheckCircle size={18} className="text-[#2563eb] shrink-0 mt-0.5" aria-hidden="true" />
